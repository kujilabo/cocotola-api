...
func (m *WorkbookRepositoryMock) FindWorkbookByName(ctx context.Context, operator user.AppUser, spaceID user.SpaceID, name string) (domain.Workbook, error) {
	args := m.Called(ctx, operator, spaceID, name)
	return args.Get(0).(domain.Workbook), args.Error(1)
}
func (m *WorkbookRepositoryMock) AddWorkbook(ctx context.Context, operator user.AppUser, spaceID user.SpaceID, param domain.WorkbookAddParameter) (domain.WorkbookID, error) {
